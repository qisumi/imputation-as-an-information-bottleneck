#!/usr/bin/env bash

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "${ROOT_DIR}"

PYTHON_BIN="${PYTHON_BIN:-python}"
DEVICE="${DEVICE:-cuda:0}"
SEEDS="${SEEDS:-42 2023 2024 2025 2026}"
EPOCHS="${EPOCHS:-100}"

OUT_DIR="runlog/mem_encoder_variants"
mkdir -p "${OUT_DIR}"
mkdir -p runlog

DATASETS=("ECG" "METR-LA" "SOLAR")
BACKBONES=("conv2d" "mlp" "attention")
MISSING_TYPES=("MCAR")

public_args="--seeds ${SEEDS} --epochs ${EPOCHS} --drop_rate 0.2 --device ${DEVICE}"
cmd="${PYTHON_BIN} src/run_mtgnn.py"

for MT in "${MISSING_TYPES[@]}"; do
  mt_lower=$(echo "${MT}" | tr 'A-Z' 'a-z')

  for DS in "${DATASETS[@]}"; do
    for BB in "${BACKBONES[@]}"; do
      RESULT_CSV="runlog/mem_encoder_variants.csv"
      echo 'pred_model,dataset,imputation_model,mae,rmse,mape,kernel_size,dropout' > "${RESULT_CSV}"

      log_name="${OUT_DIR}/exp2_${DS}_${mt_lower}_${BB}.log"

      echo "==============================================="
      echo "Dataset=${DS}, missing_type=${MT}, mask_backbone=${BB}"
      echo "Log: ${log_name}"
      echo "==============================================="

      ${cmd} \
        --dataset "${DS}" \
        --expid 200 \
        --mask_layer True \
        --mask_backbone "${BB}" \
        --missing_type "${MT}" \
        --result_path "${RESULT_CSV}" \
        ${public_args} \
        > "${log_name}" 2>&1

      cp "${RESULT_CSV}" "${OUT_DIR}/exp2_${DS}_${mt_lower}_${BB}.csv"
    done
  done
done
