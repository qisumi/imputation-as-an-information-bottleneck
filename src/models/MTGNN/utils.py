...
# Source: https://github.com/nnzhan/MTGNN
...
class DataLoaderM(object):
    def __init__(
        ...
        if mask_layer:
            mask = generate_mask_mat(xs[..., 0], p=missing_rate, missing_type=missing_type)
            xs[..., 0][mask] = np.nan
...
def load_dataset(
    ...
    if imputation_method is not None:
        if mask_layer:
            ...
            if mt == "MCAR":
                category_list[2] = f'test_mcar_{file_drop_rate}'
            else:
                category_list[2] = f'test_{mt.lower()}_{file_drop_rate}'
        else:

            if mt == "MCAR":
                category_list[2] = f'test_{imputation_method}_{file_drop_rate}'
            else:
                category_list[2] = f'test_{imputation_method}_{mt.lower()}_{file_drop_rate}'
    elif mask_layer:
    ...
    for category in category_list:
        print(os.path.join(dataset_dir, category + '.npz') + ' data used')
    ...
    data['test_loader'] = DataLoaderM(
        data[f'x_{category_list[2]}'], data[f'y_{category_list[2]}'], test_batch_size)
    data['scaler'] = scaler
    return data
