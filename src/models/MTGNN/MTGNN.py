...
class gtnet(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None,
                 ...
                 seq_length=12, in_dim=1, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3,
                 layer_norm_affline=True, mask_layer_enable=False, mask_kernel_size=2, mask_dropout=0.25,
                 mask_backbone="conv2d"):
        super(gtnet, self).__init__()

        self.mask_layer_enable = mask_layer_enable
        if (self.mask_layer_enable):
            self.mask_layer = Mask_layer(feature_channel=in_dim,
    ...
    def forward(self, input, idx=None):
        ...
        if (self.mask_layer_enable):
            mask = torch.isnan(input[:, 0, :, :]).type(torch.float32)
            expanded_mask = mask.unsqueeze(1)
            input = torch.cat((input, expanded_mask), dim=1)
            input[torch.isnan(input)] = 0
            input = self.mask_layer(input)
        else:
